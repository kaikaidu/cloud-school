...
var sex="",name="",signState="",viaState="",courseId="";//性别，姓名，报名状态，通过状态,课程id
var siState = "",viState = "";
var checkedLength = 0;
var t = null;
...
function deleteById(ids){
    layer.confirm("确认删除吗?",function (index) {
        userIds = ids;
        $.ajax({
            url : "/backend/course/state/batchDelOflive" ,
            type : "post" ,
            dataType : 'json' ,
...
function initPager(pageNo,pageSize){
    ...
    laypage.render({
        ...
        jump: function(obj, first){
            selectLiveTeleacstByCourseId(obj.curr,pageSize);
        }
    });
}
//获取参数
...
function getTotalData(){
    ...
    $.ajax({
        ...
        url: '/backend/course/selectLiveTelecastCount',
        async: false ,
        beforeSend :function(xmlHttp){
            //为了清除ie页面缓存
            xmlHttp.setRequestHeader("If-Modified-Since","0");
...
//根据课程id条件查询直播课程
function selectLiveTeleacstByCourseId(pageNo,pageSize){
    checkedLength = 0 ;
    $.ajax({
        url : "/backend/course/selectLiveTeleacstByCourseId" ,
        type : "get" ,
        dataType : "json" ,
        async : false,
        data : {
        ...
        success : function(res){
            ...
            if ("success" == res.message) {
                ...
                if (null != list && list.length > 0) {
                    $(".clearfix,.clearfix,.tableWrap").show();
                    var isVerify = list[0].isVerify;
                    if (0 == isVerify) {
                        $("#siState,.examine").hide();
                    }
                    appendHtml(list);
                    $("#testPager").show();
                    //查询进度
...
var appendHtml = function(list) {
    for (var i = 0; i < list.length ; i ++) {
        ...
        switch (list[i].signState){
            case 0:
                str += '<td><span class="ct_red">审核失败</span></td>';break;
            case 1 :
                str += '<td><span>报名成功</span></td>';break;
            case 2:
                str += '<td><span>审核中</span></td>';break;
            case 4:
                str += '<td><span class="ct_green">已完成</span></td>';break;
            default:
                str += '<td></td>';
        }
        switch (list[i].viaState){
            case 0 :
                str += '<td><span class="ct_red">未通过</span></td>';break;
            case 1:
                str += '<td><span class="ct_green">已通过</span></td>';break;
            default:
                str += '<td></td>';
        }
        str += '</tr>';
...
function updateState(){
    ...
    $.ajax({
        url : "/backend/course/updateState" ,
        type : "post" ,
        dataType : "json" ,
        data : {"siState":siState,"viState":viState,"ids":ids,"courseId":courseId} ,
        success : function (res) {
            if ("success" == res.message) {
                layer.msg("修改成功");
                initPager(pageNo,pageSize);
...
function exportUser(){
    ...
    getValue();
    location.href = "/backend/course/exportUser?ids="+ids+"&courseId="+courseId+"&sex="+sex+"&name="+name+"&signState="+signState+"&viaState="+viaState;
}
