(function($){
    $.fn.Huploadify = function(opts){
        ...
        this.each(function(){
            ...
            var inputStr = '<input id="select_btn_'+instanceNumber+'" class="selectbtn" style="display:none;" type="file" name="file"';
                ...
                funDeleteFile: function(index) {
                    for (var i = 0,len=this.fileFilter.length; i<len; i++) {
                        ...
                        if (file.index == index) {
                            ...
                            //隐藏确定上传按钮
                            var length = $(".uploadify-queue-item").length;
                            var i = 0;
                            ...
                            //因循环时当前删除的元素还未隐藏，所以强制i+1校验
                            if ((i+1) == length) {
                                $("#btnB").attr("href","javascript:void(0);").addClass("Dis");
                            }
                            //删除数组中对应的集合 file.index为下标
                            delete array[file.index-1];
                            break;
                        }
