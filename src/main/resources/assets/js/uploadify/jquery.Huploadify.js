(function($){
	$.fn.Huploadify = function(opts){
		var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><img class="showImg"/><a href="javascript:void(0);" class="uploadbtn">上传</a><a href="javascript:void(0);" class="delfilebtn">删除</a></div>';
		var defaults = {
		...
		this.each(function(index, element){
			...
			var uploadManager = {
					...
					if(typeArray.length>0){
						for(var i=0,len=files.length;i<len;i++){
							...
							if(parseInt(F.formatFileSize(f.size,true))<=option.fileSizeLimit){
								...
								alert('文件 "'+f.name+'" 大小超出'+option.fileSizeLimit / 1024 +'MB限制！');
								continue;
							}
						}
					}
				...
				onProgress : function(file, loaded, total) {
					...
					var percent = (loaded / (total * 1.1) * 100).toFixed(2) +'%';
					if(option.showUploadedSize){
						...
						xhr.onreadystatechange = function(e) {
							if(xhr.readyState == 4){
								if(xhr.status == 200){
									...
									setTimeout(function(){
										// _this.find('#fileupload_'+instanceNumber+'_'+file.index).fadeOut();
										_this.find("img.showImg").attr("src", JSON.parse(xhr.responseText).data).css({'width':'100px','height':'100px','margin':'5px 0','display':'block'});
